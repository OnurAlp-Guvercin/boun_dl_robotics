...
def evaluate(model: nn.Module, loader, device: torch.device, desc: str = "eval") -> Dict[str, float]:
    ...
    denom = float(total_count * np.prod(IMG_SHAPE))
    mse = total_mse / denom
    l1 = total_l1 / denom
    psnr = 10 * math.log10(1.0 / max(mse, 1e-12))
    return {"mse": float(mse), "l1": float(l1), "psnr": float(psnr)}


def save_examples(model: nn.Module, loader, device: torch.device, out_dir: Path, n_samples: int = 8) -> None:
    if n_samples <= 0:
        return
...
def train(
    ...
    print("[RECON] training completed. Run test command separately to evaluate on test split.")
...
def test(
    ...
    save_examples(model, loaders.test, dev, out_dir / "reconstruction_samples", n_samples=num_save_samples)
