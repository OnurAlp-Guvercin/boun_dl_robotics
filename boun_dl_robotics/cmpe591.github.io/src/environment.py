...
class BaseEnv:
    def __init__(self, render_mode="gui") -> None:
        ...
        self.reset()
        self._joint_qpos_idxs = [
            int(np.asarray(self.model.joint(x).qposadr).reshape(-1)[0]) for x in self._joint_names
        ]
    ...
    def _set_ee_pose(self, position, rotation=None, orientation=None, max_iters=2000, threshold=0.01):
        ...
        qdict = {}
        for i, q_idx in enumerate(self._joint_qpos_idxs[:-1]):
            q_idx_scalar = int(np.asarray(q_idx).reshape(-1)[0])
            qdict[i] = float(qpos[q_idx_scalar])
        ...
        while max_error > threshold:
            ...
            for idx, target_joint_pos in qdict.items():
                self.data.ctrl[idx] = target_joint_pos
            if it > max_iters:
                break
